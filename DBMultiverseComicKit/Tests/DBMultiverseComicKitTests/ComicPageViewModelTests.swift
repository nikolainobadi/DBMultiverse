...
extension ComicPageViewModelTests {
    ...
    func currentPageComplete() {
        ...
        let sut = makeSUT(chapter: chapter, currentPageNumber: 5, currentPages: [pageInfo]).sut
...
extension ComicPageViewModelTests {
    ...
    func loadDataRespectsEndPageLimit() async throws {
        ...
        let chapter = makeChapter(startPage: 1, endPage: 10)
        let sut = makeSUT(chapter: chapter, currentPageNumber: 9, pagesToLoad: pagesToLoad).sut
...
extension ComicPageViewModelTests {
    ...
    func nextPageAdvancesCorrectly() {
        ...
        let chapter = makeChapter(startPage: 1, endPage: 10)
        let sut = makeSUT(chapter: chapter, currentPageNumber: 5, currentPages: [currentPageInfo]).sut
    ...
    func nextPageFromDoubleSpread() {
        ...
        let chapter = makeChapter(startPage: 1, endPage: 10)
        let sut = makeSUT(chapter: chapter, currentPageNumber: 4, currentPages: [currentPageInfo]).sut
    ...
    func nextPageStopsAtEnd() {
        ...
        let chapter = makeChapter(startPage: 1, endPage: 10)
        let sut = makeSUT(chapter: chapter, currentPageNumber: 10, currentPages: [currentPageInfo]).sut
...
private extension ComicPageViewModelTests {
    func makeSUT(
        ...
        pagesToLoad: [PageInfo] = [],
        throwError: Bool = false,
        fileID: String = #fileID,
        filePath: String = #filePath,
        line: Int = #line,
        column: Int = #column
    ) -> (sut: ComicPageViewModel, delegate: MockDelegate) {
